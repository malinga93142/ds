...
#endif
{
    if(ptr != 0)
    {
        ...
        // Overwrite the memory with random garbage
        unsigned long *garbage_ptr = (unsigned long *)ptr;
        srand((unsigned int)time(NULL));
        unsigned long biased_rvll = 10000000000UL;
        for(size_t i = 0; i < sz; ++i)
        {
            garbage_ptr[i] = (unsigned long)(rand() % 10000000000ULL);
        }
    }
}
